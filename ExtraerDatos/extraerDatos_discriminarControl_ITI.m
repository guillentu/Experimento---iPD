function x = extraerDatos_discriminarControl_ITI(str)
    clear x;
  str
  %-Varialbes--------------------------------
  ...
  x(1)._controlITI=[];%(0)Control de palanq ITI ; cant palanq (1)IZQ y (2)DER
  %-------------------------------------------------------
  ...
  while (feof(_fid)==0)
    ...
    if cargar==1
      ...
      x(_box)._nTrial=[x(_box)._nTrial _nTrial];   %A=[];
                    %B
      x(_box)._come=[x(_box)._come _come];
      x(_box)._umbral=[x(_box)._umbral _umbral];    %F umbra para elegir desde numeron random
      x(_box)._tiempoITI=[x(_box)._tiempoITI _tiempoITI];    %D reloj en cuante regresiva
      ...
      x(_box)._controlITI=[x(_box)._controlITI _controlITI];%(0)Control de palanq ITI ; cant palanq (1)IZQ y (2)DER
      cargar=0;
    endif
    %-------------------------------------------------------  
