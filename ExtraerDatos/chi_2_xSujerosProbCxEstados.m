...
hhh=figure; 
h=errorbar([1:12],_mediaXsujeto, _stdXsujeto,'*k');
set (h, "linewidth", 3);
hh=ylabel("% of cooperation");set(hh, "fontsize", 18);
%hh=title("Cooperation Mean");set(hh, "fontsize", 16);
hold on
axis ("tic[yz]", "labely[xyz]");
hb=bar([1:12],_mediaXsujeto);
set(hb,"edgecolor","black","facecolor","black");
h2=plot([1:12],_criterio*ones(1,12),'--m');
set (h2, "linewidth", 3);
plot([1:12],.5*ones(1,12),'--k');
%legend("SEM","MEAN","half",0);
t=text([1:12]-0.25, .1*ones(1,12), _txtSujetos(:,:),"fontsize",13);
hh=xlabel("Subjects");set(hh, "fontsize", 18);
axis([0 13 0 1.15])
...
hold off
name=strcat("figura_iPD_1_2_9s_13s/fig_finales/cooperation_mean_with_significant",".eps");
print(hhh, name);
