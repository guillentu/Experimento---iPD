...
_nSujetos=12;
_iniTrial=16; %trials a evaluar
_finTrial=30; %trials a evaluar
...
for j=inicio:fin
  for i=1:_nSujetos
    _coop(i)=length(find(todo.(indice(j+1,:))(i)._respuestasEXP(_iniTrial:_finTrial)==2));
  endfor
  _cooperacion=[_cooperacion; _coop];
  for i=1:_nSujetos
    _nadaAux(i)=length(find(todo.(indice(j+1,:))(i)._respuestasEXP(_iniTrial:_finTrial)==0));
  endfor
  _nada=[_nada; _nadaAux];
  
endfor
_cooperacion=_cooperacion'
_trialsOK=length(_iniTrial:_finTrial)*ones(length([inicio:fin]),_nSujetos);
...
for j=inicio:fin
  for i=1:_nSujetos
    for k=_iniTrial:_finTrial  % nºtrials x Exp.  TRAICIONAR DADO QUE
...
for i=1:12
  Q(:,:,i)=matricesQ.(indiceSujeto(i,:))./(length(_iniTrial:_finTrial)*(fin-inicio+1)-controlFallas(i));
endfor
...
%(:,:,7)=matricesQ.(indiceSujeto(7,:))./(29*(fin-inicio+1)-controlFallas(7));
...
T(:,:)=T(:,:)/length(_iniTrial:_finTrial);C(:,:)=C(:,:)/length(_iniTrial:_finTrial);P(:,:)=P(:,:)/length(_iniTrial:_finTrial);S(:,:)=S(:,:)/length(_iniTrial:_finTrial);
figure();
