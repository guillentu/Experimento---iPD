...
for i=_sujetosNocooperadores
  ...
  for j=primero:ultimo
    ...
    aux26=(probxExp.(indiceSujeto(i,:)).(indice(j+1,:)))(:,2);
    _todoJuntoNocoop=[_todoJuntoNocoop aux23];
    _todoJuntoNocoopD=[_todoJuntoNocoopD aux26];
...
chi_2_nocoop_meanC = zeros(4,1);  % envelope means datas %sacar
chi_2_nocoop_meanD = zeros(4,1);% envelope  means datas  %sacar 
for i=1:4
  ...
  chi_2_nocoop_meanC(i) = mean(_todoJuntoNocoop(i,indexNc));%sacar
  chi_2_nocoop_meanD(i) = mean(_todoJuntoNocoop(i,indexNc));%sacar
endfor
%%%%%%%%%Chi2 de ls medias entre ProbC y ProbD%%%%%%%%%%%%%%%%%
frec_teo=mean(_frecTeoCoop(:,indexC),2);
chi_2_coop= (chi_2_coop_meanC-frec_teo).^2./frec_teo + (chi_2_coop_meanD-frec_teo).^2./frec_teo
chi_2_nocoop= (chi_2_nocoop_meanC-frec_teo).^2./frec_teo + (chi_2_nocoop_meanD-frec_teo).^2./frec_teo
%%%%%%%%%%%%%%%%%%%%%%%%
...
for i=1:4
  freedom=1;%length(find(_todosFallas==0))-1;
...
% Sin correccion de Bonferroni
for i=1:4
  freedom=1;%length(find(_todosFallasNocoop==0))-1;
